...
| Part 5 Stage 5 project-completion controls | `LOCAL_VERIFIED` | Strict predecessor, final-main CI, four-job, six-gap, annotated-tag, deterministic-archive, and claim-boundary verification; pull-request readiness is `FINAL_ATTESTATION_READY`, so project completion remains false until the exact merged `main` run and `v0.2.0` release are verified |
